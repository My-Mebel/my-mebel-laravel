...
$(document).ready(function () {
            ...
            success: function (resp) {
                ...
                if (resp.discount > 0) {
                    ...
                    $(".getAttributePrice").html(
                        '<div class="price"><h4>EGP' +
                            resp.final_price +
                            '</h4></div><div class="original-price"><span>Original Price: </span><span>EGP' +
                            resp.product_price +
                            "</span></div>"
    ...
    $(document).on("click", ".updateCartItem", function () {
        ...
        if ($(this).hasClass("plus-a")) {
            // if this clicked <a> tag has the CSS class 'plus-a', this means increase quantity by 1
        ...
        if ($(this).hasClass("minus-a")) {
            // if this clicked <a> tag has the CSS class 'minus-a', this means decrease quantity by 1
            var quantity = $(this).data("qty"); // the already existing current quantity    // using Custom HTML Attributes (data-*)
            // alert(quantity);
            // new_qty = quantity + 1; // Decrease quantity by 1
            if (quantity <= 1) {
                // Making sure that quantity can never be a negative value (can never be less than zero <0)
    ...
    $("#registerForm").submit(function () {
        ...
        $.ajax({
            ...
            success: function (resp) {
                ...
                if (resp.type == "error") {
                    // if there're Validation Errors (login fails), show the Validation Error Messages (each of them under its respective <input> field)    // 'type' is sent as a PHP array key (in the HTTP response from the server (backend)) from inside the userRegister() method in Front/UserController.php
                    ...
                    // if there're no validation errors (login is successful), redirect to the Cart page    // 'type' is sent as a PHP array key (in the HTTP response from the server (backend)) from inside the userRegister() method in Front/UserController.php
            ...
            success: function (resp) {
                ...
                if (resp.type == "error") {
                    ...
                    $.each(resp.errors, function (i, error) {
                        ...
                        ); // I already did this in the HTML page in the <p> tags in the HTML in front/users/login_register.blade.php (    <p id="login-name" style="color: red"></p>    )    // This is the same as:    $('#login-' + i).css('color', 'red');    // Change the CSS color of the <p> tags
                    ...
                    // if the Validation passes / is okay but the login credentials provided by user are incorrect, login fails, and show a generic 'Wrong Credentials!' messsage    // 'type' is sent as a PHP array key (in the HTTP response from the server (backend)) from inside the userLogin() method in Front/UserController.php
                    ...
                    // if the user being authenticated/logged in is inactive/disabled/deactivated by an admin (`status` is zero 0 in `users` table), logout the user, then return them back with a message    // 'type' is sent as a PHP array key (in the HTTP response from the server (backend)) from inside the userLogin() method in Front/UserController.php
                    $("#login-error").attr("style", "color: red"); // I already did this in the HTML page in the <p> tags in the HTML in front/users/login_register.blade.php (    <p id="login-name" style="color: red"></p>    )    // This is the same as:    $('#login-' + i).css('color', 'red');    // Change the CSS color of the <p> tags
                    $("#login-error").html(resp.message); // replace the <p> tags that we created inside the user registration <form> in front/users/login_register.blade.php depending on x in their 'id' HTML attributes 'login-x' (e.g. login-mobile, login-email, ...)
                } else if (resp.type == "success") {
                    // if there're no validation errors (login is successful), redirect to the Cart page    // 'type' is sent as a PHP array key (in the HTTP response from the server (backend)) from inside the userLogin() method in Front/UserController.php
    ...
    $("#forgotForm").submit(function () {
        // When the forgot password <form> (in front/users/forgot_password.blade.php) is submitted
